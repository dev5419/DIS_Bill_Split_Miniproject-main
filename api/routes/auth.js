...
router.post('/login', async (req, res) => {
  try {
    ...
    const user = users[0];
    if (!user.password_hash) return res.status(401).json({ message: 'Invalid email or password' });
...
router.post('/guest', async (req, res) => {
  try {
    ...
    res.json({ token, user: { uid: id, email: null, displayName: guestName } });
  } catch (error) {
...
router.get('/me', authenticate, async (req, res) => {
  try {
    ...
    const user = users[0];
    res.json({ uid: user.id, email: user.email || null, displayName: user.display_name || null });
  } catch (error) {
